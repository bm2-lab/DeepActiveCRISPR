...
LF=['hct116.episgt','hek293t.episgt','hela.episgt','hl60.episgt','label.dat']
LC=[4239,4666,8101,2076,19082]
...
for j in range(0,4):
    for i in range(0,4):
        ...
        _, X_test, __, y_test = train_test_split(fRNA, label, test_size=0.2, random_state=0, stratify = label)
        ...
        print(LFILE, RFILE, (np.shape(X_train), np.shape(y_train), np.shape(X_test), np.shape(y_test)))
